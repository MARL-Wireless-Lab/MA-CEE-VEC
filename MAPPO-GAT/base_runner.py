...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        # policy network
        self.policy = Policy(self.all_args, self.envs.observation_dim, self.envs.state_dim,
                             self.envs.action_dim, self.num_agents, device=self.device)
        # algorithm
        self.trainer = TrainAlgo(self.all_args, self.policy, device=self.device)
        # buffer
        self.buffer = SharedReplayBuffer(self.all_args, self.num_agents, self.envs.observation_dim,
                                         self.envs.state_dim, self.envs.action_dim)
    ...
    def compute(self):
        ...
        next_values = self.trainer.policy.get_values(inputs, self.buffer.rnn_states_critic[-1], self.buffer.matrix[-1])
