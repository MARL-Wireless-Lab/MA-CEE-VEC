...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_shape, state_shape, act_shape):
        ...
        self.share_obs = np.zeros((self.episode_length + 1, num_agents, state_shape),
                                  dtype=np.float32)
        ...
        self.actions_onehot = np.zeros(
            (self.episode_length + 1, num_agents, act_shape), dtype=np.float32)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                input = np.hstack((share_obs[ind:ind + data_chunk_length],
                                   actions_onehots[ind:ind + data_chunk_length],
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                inputs_batch.append(np.hstack((share_obs[:-1, ind],
                                   actions_onehots[:-1, ind],
                                   ids[:, ind])))
