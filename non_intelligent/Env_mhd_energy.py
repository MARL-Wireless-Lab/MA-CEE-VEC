...
random.seed(1103)
np.random.seed(1103)
torch.manual_seed(1103)            # 为CPU设置随机种子
...
class AUTOVANET_CV1(gym.Env):
    ...
    def update_fast_fading(self):
        ...
        fast_fading = np.abs(h) ** 2  # 20 * np.log10(np.abs(h))
        return fast_fading

    def get_path_loss_V2I(self, dist):
        path_loss = 128.1 + 37.6 * np.log10(math.sqrt(dist ** 2 + (self.h_bs - self.h_ms) ** 2) / 1000)  # dist in km
    ...
    def step(self):
        ...
        for i in range(self.car_num):
            if self.min_d:
                actions = 0
                # actions = np.random.randint(self.P_j_level - 1)
            elif self.random:
                actions = np.random.randint(self.action_dim)
            action[i][0] = actions // self.P_j_level
            action[i][1] = actions % self.P_j_level
        done = False
                ...
                if energy_up > self.energy_thre:
                    ...
                    for g in range(self.RSU_num):
                        if self.RSU_location[g, 0] == self.state[i, 3 * action[i][0]]:
                            if self.RSU_location[g, 1] == self.state[i, 3 * action[i][0] + 1]:
                                break
                    delay_exe = round(input_size * compu_intensity / (
                                (self.state[i, 3 * action[i][0] + 2] / 30) / select_RSU[g] * 1e9), 1)
                    energy_exe = self.energy_para * (((self.state[i, 3 * action[i][0] + 2] / 30) / select_RSU[
                        g] * 1e9) ** 2) * compu_intensity * input_size
                    if delay_up + delay_exe > self.delay_thre or energy_up + energy_exe > self.energy_thre:
                        ...
                        if delay_up + delay_exe > self.delay_thre:
                            ...
                            com_size = delay_remain * ((self.state[i, 3 * action[i][0] + 2] / 30) / select_RSU[
                                g] * 1e9) / compu_intensity
                            energy[i] = energy_up + self.energy_para * (((self.state[i, 3 * action[i][0] + 2] / 30) /
                                                                         select_RSU[
                                                                             g] * 1e9) ** 2) * compu_intensity * com_size
        ...
        if success == 0:
            ...
            reward = (Data_bit / self.data_max) / (Energy / (self.energy_thre * scalar_para)) - 2 * (
                        self.car_num - success)

        reward = reward / self.car_num
        Data_bit = Data_bit / self.car_num  # Mbit
        Delay = Delay / self.car_num
        Energy = Energy / self.car_num
        return self.state, reward, Data_bit / 1048576, Energy, success / self.car_num, done, Delay, delay_success / self.car_num, energy_success / self.car_num

    # raise Exception("Env error, can't go here!!")
